    ...
    async fn generate_text(&self, messages: Vec<Message>) -> AgentResult<String> {
        ...
        if content.is_empty() {
            return Err(AgentError::AiApi(
                "Claude API returned no content".to_string(),
            ));
        }
    ...
    async fn chat_completion_with_functions(
        ...
        if let Some(tool_calls) = response_data.tool_calls {
            if !tool_calls.is_empty() {
                ...
                let arguments = serde_json::to_string(&tool_call.input).map_err(|e| {
                    AgentError::AiApi(format!("Failed to serialize function arguments: {}", e))
                })?;
